...
function s:getTaggedFileList(tagfile)
  ...
  "call map(l9#readFile(a:tagfile), 'fnamemodify(v:val, ":p")') "xyf
  let result = map(result, 'fnamemodify(v:val, ":p")')
  cd -
  "call map(l9#readFile(a:tagfile), 'fnamemodify(v:val, ":~:.")') "xyf
  let result = map(result, 'fnamemodify(v:val, ":~:.")')
...
function s:handler.onModeEnterPre()
  let self.tagFiles = fuf#getCurrentTagFiles('taggedfile')
endfunction
