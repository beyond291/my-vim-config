...
"Added by xyf
let g:AutoCompleteEnable = 0
let s:enumeratingLimit = 0
...
function fuf#getCurrentTagFiles(mode)
    "Added by xyf
    if a:mode == 'taggedfile'
        let s:cur_dir = getcwd()."/"
        " setlocal tags=./.project/filenametags
        " echomsg ":setlocal tags=".s:cur_dir.eval(g:LookupFile_TagExpr)
        exec ":setlocal tags=".s:cur_dir.eval(g:LookupFile_TagExpr)
    endif
    "return sort(filter(map(tagfiles(), 'fnamemodify(v:val, '':p'')'), 'filereadable(v:val)'))
    let ret = sort(filter(map(tagfiles(), 'fnamemodify(v:val, '':p'')'), 'filereadable(v:val)'))

    "Added by xyf
    if a:mode == 'taggedfile'
        set tags < "Added by xyf 
    endif
    return ret
endfunction
...
function fuf#launch(modeName, initialPattern, partialMatching)
  ...
  "if a:modeName == 'taggedfile'
      "setlocal tags=filenametags "Added by xyf 
      "setlocal tags=filenametags
  "endif
  let s:enumeratingLimit = g:fuf_enumeratingLimit
  if a:modeName == 'buffertag' || a:modeName == 'quickfix' || a:modeName == 'jumplist'
      let g:fuf_enumeratingLimit = g:my_enumeratingLimit
  endif
  call s:runningHandler.onModeEnterPre()
  "if a:modeName == 'taggedfile'
      "setlocal tags=tags "Added by xyf
      "set tags <
  "endif
...
function s:activateFufBuffer()
  ...
  elseif exists(':AutoComplPopLock')
    AutoComplPopLock
  elseif exists(':NeoComplCacheLock') "Added by xyf
    NeoComplCacheLock
  endif
endfunction
...
function s:deactivateFufBuffer()
    ...
    elseif exists(':AutoComplPopUnlock')
        AutoComplPopUnlock
    elseif exists(':NeoComplCacheUnlock') "Added by xyf
        NeoComplCacheUnlock
    endif
    
    "Added by xyf
    if s:enumeratingLimit != g:fuf_enumeratingLimit
        let g:fuf_enumeratingLimit = s:enumeratingLimit
    endif
